...
export interface TestReport {
  ...
  tenant_id?: string
  recipe_id: string
  report_type: 'ITT' | 'System3' | 'FPC' | 'External'
  avcp_system?: '3' | '4'
  
  // Labor Information
  test_lab: string
  test_lab_address?: string
  notified_body_number?: string // Bei System 3
  ...
  async createTestReport(report: Partial<TestReport>): Promise<TestReport> {
    try {
      ...
      if (report.test_results?.fire_classification && 
        ...
        report.avcp_system = '3'
        if (!report.notified_body_number) {
          throw new Error('Notified Body Number ist erforderlich für System 3 (Brandklasse != A1fl)')
        }
      } else {
